...
t = np.arange(0.0, 4 / frequency, delta_t)
...
class Solver:
    def __init__(self):
        ...
        self.Ez_to_save = np.zeros((len(t),) + self.Ez.shape, dtype=np.float32)
        self.Hx_to_save = np.zeros((len(t),) + self.Hx.shape, dtype=np.float32)
        self.Hy_to_save = np.zeros((len(t),) + self.Hy.shape, dtype=np.float32)

        print("time step count:", len(t))
        print("Ez shape", self.Ez.shape)
        print(self.Ez_to_save.nbytes / 1.25e8, "Gb")
    ...
    def update(self, time_index):
        self.Jz[yidx_Jz, xidx_Jz] = Jz_xidx_yidx[time_index]
        self.Ez[1:-1, 1:-1] = (
            1.0
            / beta[1:-1, 1:-1]
            * (
    ...
    def animate(self):
        self.ani = animation.FuncAnimation(
            self.fig, self.update, frames=len(t), interval=1, blit=False, repeat=False
        )
        plt.show()
        data_dict = {
            "Ez": self.Ez_to_save,
        ...
        np.savez_compressed("sim_data.npz", **data_dict)
